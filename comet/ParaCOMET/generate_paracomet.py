...
def main():
    ...
    config.PRED_FILE = str(os.environ.get("PRED_FILE", "data/paracomet/gold_set.jsonl"))
    ...
    if config.DO_PRED:
        ...
        pred_dataset = read_jsonl_lines(config.PRED_FILE)
        ...
        relations = [NL_to_rel[item['prefix']] for item in pred_dataset]

        df = pd.DataFrame({"head_event":input_stories, "relation":relations})
        df['head_event'] = df['head_event'] + ' ' + df['relation'] + ' [GEN]'
        df['tail_event'] = ''
