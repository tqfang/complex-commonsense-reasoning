...
def token_wise_scoring(sequences, label_ids, attention_mask, tokenizer, device, model, max_length=150):
    ...
    for i in range(len(sequences)):
        ...
        if len(tmp_seq_list) < max_length:
            loss = get_lm_score(model, tmp_seq_list, tmp_label_list, tmp_attention_mask)
        else:
            loss = []
            for chunk in range(0, len(tmp_seq_list), max_length):
                loss.append(get_lm_score(model, tmp_seq_list[chunk:chunk + max_length],
                                         tmp_label_list[chunk:chunk + max_length],
                                         tmp_attention_mask[chunk:chunk + max_length]))
...
def score_task(question, choices, tokenizer, device, model, max_length=150):
    ...
    prediction = token_wise_scoring(sequences, label_ids, attention_mask, tokenizer, device, model, max_length)
    return prediction
...
def main():
    ...
    parser.add_argument("--max_length", default=60, type=int, required=True, help="max length")
    parser.add_argument('--overwrite_output_dir', action='store_true', help='whether to overwrite')
    ...
    if os.path.exists(out_dir) and os.listdir(out_dir) and not args.overwrite_output_dir:
    ...
    with open(out_file, "w") as f_out:
        with open(args.dataset_file) as f_in:
            for line in tqdm.tqdm(f_in):
                ...
                prediction = score_task(question, choices, tokenizer, device, model, args.max_length)
