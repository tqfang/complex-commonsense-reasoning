...
eval_tasks = [
    ...
    ("anli", "../../../data/mcqa/eval/anli_dev.jsonl"),
    ("complex2i", "../../../data/mcqa/eval/complex2i_v1.0.jsonl")
]
...
class CQAInstanceReader(InstanceReader):
    """
    Reads the CQA dataset into a unified format with context, question, label, and choices.
    """
    ...
    def to_uniform_fields(self, fields):
        question = fields['context']
        label = fields['correct']
        choices = [fields['candidates'][0], fields['candidates'][1], fields['candidates'][2], fields['candidates'][3], fields['candidates'][4]]
        return '', question, label, choices
...
INSTANCE_READERS = {"socialiqa": SocialIQAInstanceReader,
                    ...
                    'cqa': CQAInstanceReader,
                    'cwwv': CWWVInstanceReader}


def token_wise_scoring(sequences, label_ids, attention_mask, tokenizer, device, model, max_seq_length):
    ...
    for i in range(len(sequences)):
        ...
        if len(tmp_seq_list) < max_seq_length:
            loss = get_lm_score(model, tmp_seq_list, tmp_label_list, tmp_attention_mask)
        else:
            loss = []
            for chunk in range(0, len(tmp_seq_list), max_seq_length):
                loss.append(get_lm_score(model, tmp_seq_list[chunk:chunk + max_seq_length],
                                         tmp_label_list[chunk:chunk + max_seq_length],
                                         tmp_attention_mask[chunk:chunk + max_seq_length]))
...
def score_task(question, choices, tokenizer, device, model, max_seq_length):
    ...
    prediction = token_wise_scoring(sequences, label_ids, attention_mask, tokenizer, device, model, max_seq_length)
    return prediction
...
def main(args):
    ...
    if not args.overwrite_output_dir and os.path.exists(out_dir) and os.listdir(out_dir):
    ...
    with open(out_file, "w") as f_out:
        with open(args.dataset_file) as f_in:
            for line in tqdm.tqdm(f_in):
                ...
                prediction = score_task(question, choices, tokenizer, device, model, max_seq_length)
    ...
    if args.reader == "cqa":
        ...
        preds = [json.loads(line.strip())for line in open(out_file).readlines()]
        df = pd.read_csv("/nlpdata1/home/tfang/projects/complex_reasoning_atomic/AMT/results/cqa_atomic_v1.0_name.csv")
        ...
        break_down_results = evaluate_result([item['prediction'] for item in preds ], [item['correct'] for item in preds], df['type'])
        results.append(json.dumps(break_down_results))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--max_length", default=80, type=int, required=True, help="max length")
